...
def assert_strategy_basic_attributes(strategy, rollover, trailing, slippage_ticks):
    """
    Assert common strategy attributes are set correctly.

    Args:
        strategy: Strategy instance to validate
        rollover: Expected rollover value
        trailing: Expected trailing stop value
        slippage_ticks: Expected slippage ticks value

    Example:
        strategy = RSIStrategy(...)
        assert_strategy_basic_attributes(strategy, False, None, 1)
    """
...
def assert_trailing_stop_configured(strategy, trailing_value):
    """
    Assert strategy has trailing stop properly configured.

    Args:
        strategy: Strategy instance to validate
        trailing_value: Expected trailing stop percentage

    Example:
        assert_trailing_stop_configured(strategy, 2.0)
    """
    assert strategy.trailing == trailing_value
    assert strategy.trailing_stop_manager is not None


def assert_rollover_configured(strategy):
    """
    Assert strategy has rollover handling properly configured.
...
def assert_strategy_name_contains(name, *expected_substrings):
    """
    Assert strategy name contains all expected substrings.

    Args:
        name: Strategy name string
        *expected_substrings: Variable number of substrings to check for

    Example:
        name = RSIStrategy.format_name(...)
        assert_strategy_name_contains(name, 'RSI', 'period=14', 'rollover=False')
    """
    for substring in expected_substrings:
        assert substring in name, f"Expected '{substring}' in strategy name '{name}'"
...
def assert_indicator_columns_exist(df, *column_names):
    """
    Assert DataFrame contains specified indicator columns.

    Args:
        df: DataFrame to check
        *column_names: Variable number of column names to check for

    Example:
        df = strategy.add_indicators(data)
        assert_indicator_columns_exist(df, 'rsi', 'ema_short', 'ema_long')
    """
...
def assert_faster_params_generate_more_signals(signals_fast, signals_slow, param_name="parameter"):
    """
    Assert that faster parameters generate more trading signals.

    Args:
        signals_fast: DataFrame with fast parameter signals
        signals_slow: DataFrame with slow parameter signals
        param_name: Parameter name for error messages

    Example:
        df_fast = strategy_fast.generate_signals(...)
        df_slow = strategy_slow.generate_signals(...)
        assert_faster_params_generate_more_signals(df_fast, df_slow, "EMA period")
    """
    fast_count = (signals_fast['signal'] != 0).sum()
    slow_count = (signals_slow['signal'] != 0).sum()

    assert fast_count >= slow_count, \
        f"Faster {param_name} should generate more signals " \
        f"({fast_count} vs {slow_count})"


def assert_faster_params_generate_more_trades(trades_fast, trades_slow, param_name="parameter"):
    """
    Assert that faster parameters generate more trades.

    Args:
        trades_fast: Trade list from fast parameter strategy
        trades_slow: Trade list from slow parameter strategy
        param_name: Parameter name for error messages

    Example:
        trades_fast = strategy_fast.run(...)
        trades_slow = strategy_slow.run(...)
    ...
    assert len(trades_fast) >= len(trades_slow), \
        f"Faster {param_name} should generate more trades " \
        f"({len(trades_fast)} vs {len(trades_slow)})"


# ==================== Common Test Data Generators ====================

def get_common_test_params_trailing_rollover():
    """
    Get common test parameter combinations for trailing/rollover tests.

    Returns:
        List of (trailing, rollover, description) tuples

    Example:
        @pytest.mark.parametrize("trailing,rollover,description",
                                 get_common_test_params_trailing_rollover())
        def test_strategy_features(trailing, rollover, description):
            ...
    """
    return [
        (None, False, "standard"),
        (2.0, False, "with_trailing"),
        (None, True, "with_rollover"),
        (2.0, True, "trailing_and_rollover"),
    ]


def get_common_backtest_configs():
    """
    Get common backtest configuration combinations.

    Returns:
        List of (symbol, interval, trailing, description) tuples
