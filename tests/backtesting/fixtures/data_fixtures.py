...
# ==================== Data Subset Helper Fixtures ====================

@pytest.fixture
def get_data_subset():
    """
    Factory fixture to extract subset of data by date range or row count.

    Returns:
        Function that takes (df, start_date, end_date, rows) and returns subset

    Example:
        subset_func = get_data_subset
        df_subset = subset_func(zs_1h_data, rows=100)  # Last 100 rows
        df_subset = subset_func(zs_1h_data, start_date='2024-01-01', end_date='2024-06-01')
    """

    def _get_subset(df, start_date=None, end_date=None, rows=None):
        if rows is not None:
            return df.tail(rows)
...
@pytest.fixture
def zs_1h_subset_100(zs_1h_data):
    """
    Quick fixture for 100 bars of ZS 1h data.

    Useful for fast tests that don't need full dataset.

    Returns:
        DataFrame with last 100 bars of ZS 1h data
    """
    return zs_1h_data.tail(100)


@pytest.fixture
def zs_1h_subset_500(zs_1h_data):
    """
    Quick fixture for 500 bars of ZS 1h data.

    Medium-sized subset for tests that need substantial but not full data.

    Returns:
        DataFrame with last 500 bars of ZS 1h data
    """
    return zs_1h_data.tail(500)
