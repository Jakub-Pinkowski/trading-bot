...
**Problem**: Same indicator calculations are repeated across:

- Multiple strategies using same indicator
- Same strategy with different parameters
- Re-running tests with skip_existing=True

**Example Savings**:

```
Without Caching:
Strategy 1: RSI(14) on ZS_1h → Calculate
Strategy 2: RSI(14) on ZS_1h → Calculate again (duplicate!)
Strategy 3: RSI(7) on ZS_1h → Calculate

With Caching:
Strategy 1: RSI(14) on ZS_1h → Calculate & Cache
Strategy 2: RSI(14) on ZS_1h → Retrieve from cache (instant!)
Strategy 3: RSI(7) on ZS_1h → Calculate & Cache
```
...
cache_key = ('rsi', hash_series(prices), period)
...
- `(indicator_name, data_hash, ...parameters)`
- Example: `('rsi', 'abc123...', 14)`
- Data hash ensures same data = same result

**Performance Impact**:

- First calculation: Normal speed
- Subsequent accesses: ~100x faster
- Typical mass test: 50-80% cache hit rate

### Why Cache DataFrames?

**Problem**: Loading parquet files is I/O intensive.

**Solution**: LRU cache with max 50 DataFrames in memory.
...
- Multiple strategies test same data without reload
- LRU eviction keeps memory usage bounded

### Why Use ProcessPoolExecutor?

**Problem**: Python's Global Interpreter Lock (GIL) limits threading.

**Solution**: True parallelism with separate processes.
...
├── Saves caches (only once!)
└── Handles progress reporting

Worker Process 1:
├── Independent Python interpreter
├── Own cache instances (in-memory)
└── No GIL contention
...
- Workers load cache at startup
- Workers use cache during execution
- Workers DO NOT save cache (avoided to prevent conflicts)
- Main process saves cache after all workers complete

**Performance**:

- Linear speedup with CPU cores
- 8 cores ≈ 7.5x faster than single-threaded
- Overhead: ~5% for process management

### Why Validate DataFrames?

**Problem**: Bad data causes cryptic errors deep in strategy execution.
...
Data Quality:
├── Check for >10% NaN values (warning)
├── Verify index is sorted (error if not)
└── Check for duplicate timestamps (warning)

Minimum Rows:
└── Warn if < MIN_ROWS_FOR_BACKTEST (150)
    ├── 100 rows for indicator warm-up
    └── 50 rows for meaningful results
```
...
- Prevents cascade failures in workers

### Why Skip Indicator Warm-up Period?

**Problem**: Indicators need historical data to stabilize.

**Example - RSI(14)**:

```
Bar 1: Not enough data (need 14 bars)
Bar 14: First RSI value (unstable)
Bar 50: More stable
Bar 100: Fully stabilized
```

**Solution**: Skip first 100 candles for all trades.
...
for idx, row in df.iterrows():
    ...
    # Process signals only after warm-up
    ...
```

**Benefits**:

- Prevents false signals from unstable indicators
- Ensures all strategies start with valid data
- Matches real-world trading (wait for confirmation)

### Why Pre-process Switch Dates?

**Problem**: Parsing dates repeatedly in each worker is wasteful.

**Solution**: Parse once in main process, pass to workers.

```python
# Main process (once):
preprocessed_switch_dates = {
    symbol: [pd.to_datetime(date) for date in dates]
    for symbol, dates in switch_dates_dict.items()
}

# Worker process (reuses):
trades = strategy.run(df, preprocessed_switch_dates[symbol])
```

**Benefits**:

- Reduces worker startup time
- Eliminates redundant datetime parsing
- ~2-3% overall speedup
...
1. **Realistic Simulation**: Matches how live trading works
2. **Prevents Look-ahead Bias**: Cannot use future information
3. **Accounts for Execution Delay**: Natural lag between signal and fill
4. **Slippage Handling**: Entry price is next bar open (may gap)
...
```python
indicator_cache = {
    ('rsi', 'hash_abc123', 14): pd.Series([...]),
    ('ema', 'hash_def456', 9): pd.Series([...]),
    ('macd', 'hash_abc123', 12, 26, 9): {
        'macd': pd.Series([...]),
        'signal': pd.Series([...]),
        'histogram': pd.Series([...])
    },
...
1. **Startup**: Load from disk (if exists)
2. **Runtime**: Check before each calculation
3. **Shutdown**: Save to disk (main process only)
...
```python
dataframe_cache = {
    '/path/to/ZS_1h_2023-01.parquet': pd.DataFrame(...),
    '/path/to/GC_4h_2023-02.parquet': pd.DataFrame(...),
    ...
}
```
...
1. **Startup**: Load from disk (if exists)
2. **Runtime**: Check before each file read
3. **Shutdown**: Save to disk (main process only)

**Eviction Policy**:

- LRU (Least Recently Used)
- Max size: 50 DataFrames
- Max age: 7 days

### Why Workers Don't Save Caches

**Problem**: Multiple workers saving simultaneously causes:

- File corruption (race condition)
- Disk I/O bottleneck
- Inconsistent cache state

**Solution**: Only main process saves caches
...
with ProcessPoolExecutor(max_workers=max_workers) as executor:
    # Workers run tests (use caches, but don't save)
    for future in as_completed(future_to_test):
        result = future.result()
        self.results.append(result)

# After all workers complete (main process only):
...
### 1. Pre-allocation of Result Arrays

**Problem**: Appending to lists in loop is slow.

**Solution**: Pre-allocate arrays.

```python
# In _results_to_dataframe():
...
data = {
    ...
    'win_rate': [0.0] * n_results,
    ...
}

# Fill directly (no append)
for i, result in enumerate(self.results):
    data['month'][i] = result['month']
    data['win_rate'][i] = result['metrics']['win_rate']
    ...
```

**Speedup**: 5-10x faster for large result sets

### 2. Efficient Existing Test Lookup

**Problem**: Checking if test exists is O(n) with list.
...
# O(1) lookup
if (month, symbol, interval, strategy) in existing_combinations:
    # Skip this test
    ...
```

**Speedup**: 100x faster for large result sets

### 3. Periodic Garbage Collection

**Problem**: Memory usage grows over time.

**Solution**: Force GC every 100 tests.
...
**Impact**: Keeps memory usage stable

### 4. Intermediate Result Saves

**Problem**: If mass test crashes, all results lost.

**Solution**: Save every 1000 tests.
...
**Impact**: Minimal data loss on crash
...
The system is designed to continue operating even when individual tests fail:
...
try:
    ...
    logger.exception(f'Worker exception: {test_details}')
    failed_tests += 1
    continue  # Don't crash entire run
```
...
The backtesting architecture balances:

- **Performance**: Parallel processing + aggressive caching
- **Accuracy**: Realistic signal execution + comprehensive validation
- **Reliability**: Graceful error handling + intermediate saves
- **Maintainability**: Clear separation of concerns + extensive documentation

This design enables efficient testing of thousands of strategy variants while maintaining the fidelity needed for
